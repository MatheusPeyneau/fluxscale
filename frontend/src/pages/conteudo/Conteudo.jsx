            ...
            <div className="bg-card border border-border rounded-xl p-6" data-testid="agent3-result">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h2 className="text-base font-heading font-semibold flex items-center gap-2">
                    <CheckCircle2 size={16} className="text-emerald-500" />
                    Copy criado — {copyData.slides?.length || 0} slides
                  </h2>
                  {copyData.hook && (
                    <p className="text-xs text-muted-foreground mt-0.5 italic">"{copyData.hook}"</p>
                  )}
                </div>
                <Button variant="outline" size="sm" onClick={runAgent3} data-testid="retry-agent3">
                  <RotateCcw size={12} className="mr-1.5" />Regenerar
                </Button>
              </div>

              <div className="space-y-2 mb-4 max-h-72 overflow-y-auto pr-1">
                {(copyData.slides || []).map((slide, i) => (
              ...
              <div className="flex items-center gap-3">
                <Button variant="ghost" size="sm" onClick={() => setStep("agent2_done")}>Voltar</Button>
                <Button onClick={runAgent4} className="gap-2" data-testid="proceed-to-design">
          ...
          {step === "agent4_done" && htmlContent && (
            <div data-testid="agent4-result">
              <div className="flex items-center justify-between mb-4">
                ...
                <Button variant="outline" size="sm" onClick={runAgent4} data-testid="retry-agent4">
                  <RotateCcw size={12} className="mr-1.5" />Regenerar
                </Button>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-3">
                <Button variant="ghost" size="sm" onClick={() => setStep("agent3_done")}>Voltar</Button>
                <Button
                  variant="outline"
                ...
                <Button
                  ...
                  disabled={saved}
                  className="gap-2"
                  data-testid="save-history-button"
                >
                  {saved
                    ? <><CheckCircle2 size={14} />Salvo!</>
                    : <><Save size={14} />Salvar no Histórico</>}
                </Button>
                <Button variant="ghost" size="sm" onClick={resetFlow} data-testid="new-carousel-button">
                  <RotateCcw size={13} className="mr-1.5" />Novo carrossel
                </Button>
              </div>
            </div>
          )}
