...
function CarouselPreviewSection({
  ...
  // After iframe loads, try to detect total slides from the HTML
  const onIframeLoad = () => {
    try {
      const iwin = iframeRef.current?.contentWindow;
      if (!iwin) return;
      // Common patterns generated by the LLM
      const slides = iwin.document.querySelectorAll('[class*="slide"], [id*="slide"]');
      if (slides.length > 1) setTotalSlides(slides.length);
    } catch {}
  };
  ...
  const navigateSlide = (dir) => {
    ...
    try {
      if (dir === "next") {
        if (typeof win.nextSlide === "function") win.nextSlide();
        else if (typeof win.showNext === "function") win.showNext();
        else if (typeof win.next === "function") win.next();
        else {
          // Fallback: look for next button inside iframe
          const btn = win.document.querySelector('[onclick*="next"], .btn-next, .next-btn, [data-next]')
            || [...win.document.querySelectorAll("button")].find(b => b.textContent.includes("→") || b.textContent.toLowerCase().includes("próx") || b.textContent.includes(">"));
          btn?.click();
        }
        setCurrentSlide(p => Math.min(p + 1, totalSlides));
      } else {
        if (typeof win.prevSlide === "function") win.prevSlide();
        else if (typeof win.showPrev === "function") win.showPrev();
        else if (typeof win.prev === "function") win.prev();
        else {
          const btn = win.document.querySelector('[onclick*="prev"], .btn-prev, .prev-btn, [data-prev]')
            || [...win.document.querySelectorAll("button")].find(b => b.textContent.includes("←") || b.textContent.toLowerCase().includes("ant") || b.textContent.includes("<"));
          btn?.click();
        }
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden mb-4" data-testid="html-preview-container">
        ...
        <div className="bg-muted/50 border-b border-border px-4 py-2 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="text-xs text-muted-foreground font-medium">Preview do Carrossel</span>
            <div className="flex items-center gap-1">
              ...
              <span className="text-xs text-muted-foreground px-1 min-w-[48px] text-center">
                Slide {currentSlide}{totalSlides > 1 ? ` / ${totalSlides}` : ""}
              </span>
              <button
